...
const ToneColor = createContext(DEFAULT_COLOR)
...
function Base({
  ...
  return (
    <ToneColor.Provider value={color}>
      <Svg
        width={size}
        height={size}
        viewBox="0 0 24 24"
...
function Tone({ d }: { d: string }) {
  const color = useContext(ToneColor)
  return <Path d={d} fill={color} stroke="none" opacity={0.15} />
}
...
export function IconContrast(props: IconProps) {
  ...
  return (
    <Base {...props}>
      <Path d="M12 4.5a7.5 7.5 0 0 1 0 15Z" fill={color} stroke="none" opacity={0.3} />
      <Circle cx="12" cy="12" r="7.5" />
      <Path d="M12 4.5v15" />
    </Base>
  )
