 ...
 * when the table is still empty and points at the single thing worth doing
 * next. It never counts, never scolds and never dramatises a gap
 * (BRAND.md voice rules, docs/09 invariants).
 *
 ...
 *
 * Pure on purpose: the whole ladder is decided here so every branch can be
 * unit tested, and the screen only renders what comes back.
 */
...
export function chooseAfiMoment(input: AfiMomentInput): AfiMoment | null {
  if (input.neverLogged) return null
  ...
  // Every core group made it to the plate. Nothing about today outranks that.
  if (meals > 0 && gaps === 0) {
    return waterKnown && waterGap === 0
      ? {
          key: 'denge-ve-su',
          pose: 'kutlama',
          motion: 'zafer',
  ...
  if (meals === 0) {
    ...
    // A run in progress is worth saying out loud; the invitation stays the same.
    return streak >= RHYTHM_FROM
      ? {
  ...
  /* The plate is started but not complete. Named without a verdict first: a
     sweet day is a day, not a mistake. */
  if (sweetish >= 2) {
    return {
      key: 'tatli-gunu',
      pose: 'temel',
      motion: 'idle',
      line: 'Bugün tatlı tarafı ağır bastı, olur böyle. Sofrada renge hep yer var.',
      accent: 'amber',
      action: 'meal',
    }
  }

  /* Then one nudge, and it is whichever is further from done: a note that asks
     for two things at once stops being a companion and becomes a list. */
  if (waterKnown && waterGap > 0 && waterGap >= gaps / 5) {
    return glasses === 0
      ? {
          key: 'su-yok',
          pose: 'su',
  ...
  // Whatever is left is a gap on the plate; `gaps` is above zero by here.
  return {
    key: 'denge-eksik',
    pose: 'merak',
    motion: 'idle',
