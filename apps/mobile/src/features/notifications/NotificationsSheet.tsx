 ...
 *
 * Zil birincil kanal olduğu için liste artık üç şeyi birden taşıyor: selamlar,
 ...
 *
 * Okundu kalem başınadır: sheet'in AÇILMASI hiçbir şeyi okumuş saymaz, çünkü
 * artık listede tek tek okunmayı hak eden kalemler var. Dokunmak okur; toplu
 * işaret başlıktaki ayrı eylemdir.
 */
export function NotificationsSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  /* Dokunmak her zaman okur, gidecek yer olsun olmasın: metinden ibaret bir
     kutlamanın da okunabilmesi gerekiyor. Hedef jetonu push'un kullandığı
     kümeden geçer (push-target.ts), yani zil de uygulamanın önceden seçtiği
     yerlerden başka bir yere gidemez. 'notifications' hedefi zaten burasıdır:
     o kalemde yalnız sheet kapanır. */
          ...
          {items.map((n) => {
            ...
            return (
              <Pressable
                ...
                className="rounded-2xl bg-canvas p-4 active:opacity-80"
              >
                <View className="flex-row items-center gap-3">
                  ...
                  <View className="min-w-0 flex-1">
                    ...
                    {n.detail ? (
                      <AppText className="mt-0.5 text-xs text-soft">{n.detail}</AppText>
                    ) : null}
                    {dayLabel ? (
                  ...
                  {n.read ? null : <View className="h-2 w-2 rounded-full bg-orange-500" />}
                </View>
